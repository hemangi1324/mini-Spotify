...
class Library
{
	...
	Songs* findInList(Songs* head, const string& name, bool isCircular = false) {
		if (!head) return nullptr;
		Songs* temp = head;
		do {
			if (temp->title == name) return temp;
			temp = temp->next;
	...
	void play(const string& name) {
		Songs* song = findInList(root, name, true);
		if (!song) song = findInList(head, name);
		if (!song) { cout << "No such song found :(" << endl; return; }

		st.push(song);
		cout << "Playing " << song->title << " by " << song->singer << "....." << endl;
		sleep(song->duration);
	}

	void NextSong() {
		if (st.empty()){
			cout << "First play a song!!" << endl; return; 
		}
		Songs* nextS = st.top()->next;
		if (!nextS){
		 cout << "No next song available!" << endl; return;
		}
		st.push(nextS);
		cout << "Playing next song: " << nextS->title << "....." << endl;
	...
	void PreviousSong() {
		...
		if (!prevS){
			cout << "No previous song available!" << endl; return; 
		}

		st.push(prevS);
		cout << "Playing previous song: " << prevS->title << "....." << endl;
